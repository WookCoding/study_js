// JS 의 Array객체는 길이를 설정하지 않아도 원하는 인덱스에 원하는 값을 바로 추가할 수 있다.
...
// slice(begin) : begin부터 마지막 인덱스 까지 추출
...
datas.map(function(data){return data / 2 }).forEach(data => console.log(parseInt(data)));

// split("구분점")
"월화수목금토일".split("").forEach(e => console.log(e));

let dayOfWeek = new Array();
// let dataOfWeek = new Array(7);
"월화수목금토일".split("").forEach(data => dayOfWeek.push(data));

dayOfWeek = new Array(7);
dayOfWeek.fill("");
"월화수목금토일".split("").forEach(function(data, i, datas){
    dayOfWeek[i] = data;
});
